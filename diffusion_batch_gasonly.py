...
print(inspect.signature(ScoreModel.fit))

import torch

# Setup (once, at start of training)
device = torch.device("cuda")
amp_dtype = torch.bfloat16  # H100-preferred
...
parser.add_argument('--checkpoint_file', type=str, default='checkpoint_8.9659e+04_010.pt',
                    help='Checkpoint filename')
parser.add_argument('--checkpoint_number', type=int, default=10,
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, start_idx, length, ic_stats=None, fd_stats=None):
        ...
        all_fds = np.load(f'/work/hdd/bdne/abonab/downsampled_fd_64_hydrogas_norm.npy') ##### CHANGED
...
if args.load_checkpoint:
    ...
    losses = model.fit(train_dataset, epochs=100, batch_size=B, learning_rate=1e-4,
                       checkpoints_directory=f'/work/hdd/bdne/abonab/run_51', models_to_keep=1000, val_dataset=val_dataset, model_checkpoint=checkpoint_number)
    end_time = time.time()
