...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, start_idx, length, ic_stats=None, fd_stats=None):
        ...
        all_ics = np.load(f'/work/hdd/bdne/abonab/ic_all_norm.npy') ### change
        all_fds = np.load(f'/work/hdd/bdne/abonab/fd_64_mcdm_norm.npy') ### change
...
if args.load_checkpoint:
    ...
    losses = model.fit(train_dataset, epochs=100, batch_size=B, learning_rate=1e-4,
                       checkpoints_directory=f'/work/hdd/bdne/abonab/run_52', models_to_keep=1000, val_dataset=val_dataset, model_checkpoint=checkpoint_number)
    end_time = time.time()
