...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, start_idx, length, ic_stats=None, fd_stats=None):
        ...
        all_ics = np.load(f'/work/hdd/bdne/abonab/downsampled_ic_64_camels_normalized_mod.npy')
        all_fds = np.load(f'/work/hdd/bdne/abonab/downsampled_fd_64_camels_normalized_mod.npy')
...
net = NCSNpp(channels=C, nf=32, ch_mult=[1, 2, 2, 1, 1], dimensions=3, dropout=0.1,
             condition=('input',), condition_input_channels=1).to('cuda')
model = ScoreModel(model=net, sigma_min=0.01, sigma_max=15, device="cuda") # increase sigma_max
...
if args.load_checkpoint:
    ...
    losses = model.fit(train_dataset, epochs=50, batch_size=B, learning_rate=1e-5,
                       checkpoints_directory=f'/work/hdd/bdne/abonab/run_38', val_dataset=val_dataset)#), model_checkpoint=10)
    end_time = time.time()
...
ic_all = np.load('/work/hdd/bdne/abonab/downsampled_ic_64_camels_normalized_mod.npy') # can replace with z=127 files
